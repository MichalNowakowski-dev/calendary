...
async function PermissionsOverview() {
  try {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {companies.filter(c => c.company_subscriptions?.[0]?.status === 'active').length}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Most Popular Module</CardTitle>
              <Users className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-lg font-bold">
                {Object.keys(moduleCounts).length > 0 
                  ? moduleNames[Object.keys(moduleCounts).reduce((a, b) => 
                      moduleCounts[a] > moduleCounts[b] ? a : b
                    ) as keyof typeof moduleNames]?.split(' ')[0] || 'None'
                  : 'None'}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Avg Modules per Company</CardTitle>
              <Shield className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">
                {companies.length > 0 
                  ? (Object.values(moduleCounts).reduce((a, b) => a + b, 0) / companies.length).toFixed(1)
                  : '0'}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="hover:shadow-lg transition-shadow cursor-pointer">
            ...
            <CardContent>
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                Override module access for specific companies beyond their plan defaults.
              </p>
              <a 
    ...
    return (
      <div className="text-center text-red-600 p-4">
        Error loading permissions overview. Please check your permissions.
      </div>
    );
  }
}
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-gray-600 dark:text-gray-400 mt-2">
          Monitor module usage and manage access permissions across all companies
        </p>
      </div>
