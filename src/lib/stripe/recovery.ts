...
export async function getPaymentFailureInfo(
  companyId: string
): Promise<PaymentFailureInfo | null> {
  try {
    ...
    if (failedInvoice) {
      ...
      if (failedInvoice.last_finalization_error) {
        failureReason =
          failedInvoice.last_finalization_error.message || "Payment failed";
      }
    }
  ...
  try {
    ...
    if (subscription.status === "past_due") {
      ...
      return {
        ...
        actions: failureInfo ? await getRecoveryActions(failureInfo) : [],
      };
    }
    ...
    if (subscription.status === "cancelled") {
      return {
        ...
        actions: [
          {
            type: "contact_support",
            description: "Contact support to reactivate your subscription",
          },
        ],
      };
    }
